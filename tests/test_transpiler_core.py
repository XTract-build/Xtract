...
def test_type_uint256_max():
    """type(uint256).max should transpile to BigUint 256-bit max"""
    t = Transpiler()
    result = t._convert_expression("type(uint256).max")
    assert "BigUint::from_bytes_be(&[0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff])" in result, f"Got: {result!r}"


def test_type_int256_max():
    """type(int256).max should transpile to BigInt 256-bit max"""
    t = Transpiler()
    result = t._convert_expression("type(int256).max")
    assert "BigInt::from(BigUint::from_bytes_be(&[0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]))" in result, f"Got: {result!r}"


def test_type_int256_min():
    """type(int256).min should transpile to BigInt 256-bit min"""
    t = Transpiler()
    result = t._convert_expression("type(int256).min")
    assert "-BigInt::from(BigUint::from_bytes_be(&[0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]))" in result, f"Got: {result!r}"
