...
- Contains original Solidity contract files
- Example: `SimpleStorage.sol`, `ERC20Token.sol`

**Stage 2: Transpiled Output**
- Location: `test_cases/expected/*.rs`
- Generated by: `xtract/transpiler.py` (765 lines)
- Contains: MultiversX-compatible Rust code
- ✅ **This is the direct transpiler output** - no manual edits
- Example: `SimpleStorage.rs`, `ERC20Token.rs`
...
- Generated by: `sc-meta all build`
- Contains: Compiled WebAssembly bytecode ready for deployment

**Stage 5: Deployment**
- Executed by: `mxpy contract deploy`
- Result: Live contracts on MultiversX Devnet
- All deployed contracts use **direct transpiler output** - verified by file comparison

### Important Notes

- **Source of Truth**: `test_cases/expected/` contains the definitive transpiler output
- **No Manual Edits**: All deployed contracts are identical to transpiler output (verified)
- **Build Process**: `demo/` folder is temporary - used only for building and deployment
- **Verification**: File comparison confirms deployed contracts match transpiler output exactly

## Python package structure

The repository includes a minimal Python package `xtract` implementing the core transpilation pipeline:

- `xtract/transpiler.py`: core logic for parsing Solidity surface patterns and emitting MultiversX Rust (765 lines)
